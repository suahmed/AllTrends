...
public class EventProcessor<Value> {
	...
	public ArrayList<String> getEdges() {
		...
		for (Node<Value> node : this.nodes) {
			for (Node<Value> node2 : node.successors) {
				if(!node2.equals(targetNode)){
					edges.add(node.data + " " +node2.data);
				}
			}
		}
		return edges;
	...
	public ArrayList<String> getStartVertices() {
		ArrayList<String> vertices=new ArrayList<String>();

		for (Node<Value> node : sourceNode.successors) {
	...
	// output end vertices
	public ArrayList<String> getEndVertices() {
		ArrayList<String> vertices=new ArrayList<String>();

		for (Node<Value> node : targetNode.predecessors) {
			vertices.add(node.data +"");
		}
		return vertices;
	}
	...
	public void Fusion2(ArrayList<Node<Value>> nodes, int ps, PrintWriter out) {
		...
		applyDfsOnParts2();
		/*
		 * // apply optimal partitioning algorithm searchOptimal();
		 */
		// call recursive sequence construction on optimal partitions and record
		// time
		// DFS with memoization on parts and generate sequences recursively
		// without memoization
	...
	public void findCleanCuts(int limit, ArrayList<Node<Value>> nodes) {
		...
		// int nPartitions= (nodes.size()%limit == 0? nodes.size()/limit :
		// nodes.size()/limit +1);
		...
		while (j < nodes.size()) {
			...
			do {
				...
				// check all last node's successors to be same to make
				// isCleanCut=true
			...
			// cutPart.sourceNode.successors);
			// System.out.println("Predecess(T): " +
			// cutPart.targetNode.predecessors);
			...
			// HashSet<Node<Value>> lastNodePartitionSuccessors =
			// (HashSet<Node<Value>>) node.successors2.clone();
			// lastNodePartitionSuccessors.removeAll(partNodes);
			// lastNodeSuccessors.put(node, lastNodePartitionSuccessors);
			// System.out.println("Last: "+ node + " Successors:" +
			// lastNodePartitionSuccessors);
			// }
	...
	private boolean checkCleanCut(HashSet<Node<Value>> lastNodes, ArrayList<Node<Value>> partNodes) {
		...
		for (Node<Value> node : lastNodes) {
			/*
			 * if(i==0){ nextNodes = (HashSet<Node<Value>>)
			 * node.successors2.clone(); i++; } else
			 */
			{
				...
				if (lastNodePartitionSuccessors.containsAll(nextNodes) == false) { // ||
																					// nextNodes.containsAll(lastNodePartitionSuccessors)){
					return false;
				}
			}
			// System.out.println("Last: "+ node + " Successors:" +
	...
	public void genSequencesFromParts(PrintWriter out) {
		...
		if (pathCount > 0) {
			...
			// System.out.println("Estimated # of Sequences: " +
			// estimateCost());
		} else
			...
			for (int i = 0; i < curPaths.size(); i++) {
				...
				if (partIndex == partCount - 1) {
					...
					if (out != null) {
						...
						// out.println("( "+pathCount+" : "+ findSize(curSeq) +"
						// ) : " + curSeq);
