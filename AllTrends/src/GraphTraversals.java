...
public class GraphTraversals<Value> {
	...
	private void findAllPredecessors(Node<Value> currentNode, Node<Value> node, HashSet<Node<Value>> predecessors) {
		...
		// recursively visit all of unvisited predecessors
		...
		for (Node<Value> successor : currentNode.successors) {
			...
			if (currentNode != sourceNode) {
				if (paths.containsKey(successor)) {
					...
					// newpath.addFirst(successor); // can be removed and wont
					// need removeLast
	...
	void printPathCountNoDyn() {
		...
		/*
		 * if(paths.containsKey(sourceNode)){ System.out.println(
		 * "Total # of Longest Sequences: " + paths.get(sourceNode).size()); }
		 * else System.out.println("No path from "+ sourceNode + " To " +
		 * targetNode);
		 */
		if (pathCount > 0) {
	...
	void printPathsNoDyn(PrintWriter out) {
		...
		pathCount = 0;
		/*
		 * if(out!=null){ graphDFSByRecersionNoDyn(out); } else{
		 * graphDFSByRecersionNoDyn(null); }
		 */
		 ...
		 * if(paths.containsKey(sourceNode)){ System.out.println(
		 * "Total # of Longest Sequences: " + paths.get(sourceNode).size());
		 * System.out.println("Longest Sequences are:"); for (
		 * LinkedList<Node<Value>> path : paths.get(sourceNode)){
		 * //path.addFirst(sourceNode); //path.removeLast();
	...
	public void graphDFSByRecersionNoDyn(Node<Value> currentNode, PrintWriter out) {
		...
		if (currentNode.equals(targetNode)) {
			// ArrayList<LinkedList<Node<Value>>>
			// curPaths=paths.get(sourceNode);
			// curPaths.add(new LinkedList<Node<Value>>(pathSoFar));
			pathCount++;
			if (out != null) {
				// System.out.println("( "+pathCount+" : "+pathSoFar.size() +" )
				// : " + pathSoFar);
	...
	public static void testSampleGraph2() {
		...
		/*
		 * Node<Integer> S = new Node<Integer>(1); Node<Integer> A = new
		 * Node<Integer>(2); Node<Integer> B = new Node<Integer>(3);
		 * Node<Integer> C = new Node<Integer>(4); Node<Integer> D = new
		 * Node<Integer>(5); Node<Integer> E = new Node<Integer>(6);
		 * Node<Integer> F = new Node<Integer>(7); Node<Integer> G = new
		 * Node<Integer>(8); Node<Integer> H = new Node<Integer>(10);
		 * Node<Integer> T = new Node<Integer>(9);
		 * 
		 * S.connectTo(A); S.connectTo(H);
		 * 
		 * A.connectTo(B); A.connectTo(D);
		 * 
		 * B.connectTo(C); B.connectTo(F);
		 * 
		 * C.connectTo(G);
		 * 
		 * D.connectTo(C); D.connectTo(E);
		 * 
		 * E.connectTo(F);
		 * 
		 * F.connectTo(G);
